...
class OAuthModel {
  ...
  static grantTypeAllowed(clientID, grantType) {
  ...
  static getUser(username, password){
    ...
    return userDBHelper.getUserFromCrentials(username, password)
  }
  ...
  static saveAccessToken(accessToken, clientID, expires, user, callback){
    ...
    return accessTokensDBHelper.saveAccessToken(accessToken, user.id)
  }
  ...
  static getAccessToken(bearerToken) {
    ...
    return accessTokensDBHelper.getUserIDFromBearerToken(bearerToken)
      .then((userID) => {
        if (userID) {
          throw 'Unable to find user for that token';
        }
